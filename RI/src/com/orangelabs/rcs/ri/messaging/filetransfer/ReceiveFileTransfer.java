...
public class ReceiveFileTransfer extends Activity {
        ...
        public void onDeliveryInfoChanged(String chatId, ContactId contact, String transferId,
            ...
            if (LogUtils.isActive) {
                Log.d(LOGTAG, new StringBuilder("onDeliveryInfoChanged contact=").append(contact)
                        .append(" transferId=").append(transferId).append(" state=").append(state)
        ...
        try {
            ...
            if (mFileTransfer == null) {
                try {
                    ...
                    if (mFtDao.getState() == FileTransfer.State.TRANSFERRED) {
    ...
    private void onTransferStateChangedUpdateUI(final int state, final int reasonCode) {
        if (state > RiApplication.FT_STATES.length) {
            if (LogUtils.isActive) {
                Log.e(LOGTAG, "TransferStateChanged unhandled state=".concat(String.valueOf(state)));
            }
            return;
        }
        if (reasonCode > RiApplication.FT_REASON_CODES.length) {
            Log.e(LOGTAG,
                    "TransferStateChanged unhandled reason=".concat(String.valueOf(reasonCode)));
            return;
        }

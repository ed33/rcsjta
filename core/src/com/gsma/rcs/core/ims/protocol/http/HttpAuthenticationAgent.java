...
public class HttpAuthenticationAgent {
    ...
    public String generateAuthorizationHeader(String method, String requestUri, String body)
        ...
        String auth = "Authorization: "
                + generateAuthorizationHeaderValue(method, requestUri, body);
        return auth;
    }
    ...
    public String generateAuthorizationHeaderValue(String method, String requestUri, String body)
        ...
        try {
            ...
            if (!isDigestAuthentication) {
                ...
                return "Basic "
                        + Base64.encodeBase64ToString((serverLogin + ":" + serverPwd).getBytes());
            }

            digest.updateNonceParameters();

            // Build the Authorization header
            String authValue = "Digest username=\"" + serverLogin + "\"" + ",realm=\""
                    + digest.getRealm() + "\"" + ",nonce=\"" + digest.getNonce() + "\"" + ",uri=\""
                    + requestUri + "\"" + ",nc=" + digest.buildNonceCounter() + ",cnonce=\""
                    + digest.getCnonce() + "\"";
            ...
            if ((qop != null) && qop.startsWith("auth")) {
                ...
                // Calculate response
                response = digest.calculateResponse(serverLogin, serverPwd, method, requestUri,

            ...
            if (message instanceof SipRequest && ctx.isSipResponse()) {
                ...
                if (method != null && keepAliveManager != null) {
                    if (method.equals(Request.INVITE) || method.equals(Request.REGISTER)) {
                        // Message is a response to INVITE or REGISTER: analyze "keep" flag of "Via"
                        // header
                        int viaKeep = -1;
                        ...
                        // If "keep" value is invalid or not present, set keep alive period to
                        // default value
                        if (viaKeep <= 0) {
                            keepAliveManager.setPeriod(RcsSettings.getInstance()

...
public abstract class ImsServiceSession extends Thread {
     ...
     * Session invitation status
     */
    protected int mInvitationStatus = INVITATION_NOT_ANSWERED;
    ...
    public void acceptSession() {
        ...
        mInvitationStatus = INVITATION_ACCEPTED;
    ...
    public int waitInvitationAnswer() {
        if (mInvitationStatus != INVITATION_NOT_ANSWERED) {
    ...
    public void abortSession(int abortedReason) {
        if (sLogger.isActivated()) {
            sLogger.info("Abort the session " + abortedReason);
        }
        ...
        if (abortedReason == ImsServiceSession.TERMINATION_BY_USER && mDialogPath != null
        ...
        for (ImsSessionListener listener : listeners) {
            listener.handleSessionAborted(mContact, abortedReason);
        }
    }
    ...
    public void terminateSession(int reason) {
        if (sLogger.isActivated()) {
            sLogger.debug("Terminate the session (reason " + reason + ")");
        }
    ...
    public void receiveCancel(SipRequest cancel) {
        ...
        mInvitationStatus = ImsServiceSession.INVITATION_CANCELED;

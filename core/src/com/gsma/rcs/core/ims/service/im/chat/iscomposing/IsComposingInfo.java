...
public class IsComposingInfo {
    ...
    public static String buildIsComposingInfo(boolean status) {
        ...
        return "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + CRLF
                ...
                + CRLF + "<state>" + state + "</state>" + CRLF + "<contenttype>"
                + InstantMessage.MIME_TYPE + "</contenttype>" + CRLF + "<lastactive>"
                + DateUtils.encodeDate(System.currentTimeMillis()) + "</lastactive>" + CRLF
                + "<refresh>60</refresh>" + CRLF + "</isComposing>";
    }
}

...
public class RcsCoreService extends Service implements CoreListener {
     ...
     * Contacts API
     */
    private ContactsServiceImpl mContactsApi;
    ...
    private synchronized void startCore() {
        ...
        try {
            ...
            ContactsManager.createInstance(mContext, contentResolver, localContentResolver,
                    mRcsSettings);
            ...
            // Instantiate API
            mContactsApi = new ContactsServiceImpl(contactsManager, mRcsSettings);
    ...
    public IBinder onBind(Intent intent) {
        if (IContactsService.class.getName().equals(intent.getAction())) {
            if (sLogger.isActivated()) {
                sLogger.debug("Contacts service API binding");
            }
            return mContactsApi;

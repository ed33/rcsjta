...
public class GroupChatImpl extends IGroupChat.Stub implements ChatSessionListener {
    ...
    private boolean isGroupChatCapableOfReceivingParticipantInvitations() {
        if (!mRcsSettings.isGroupChatActivated()) {
            if (logger.isActivated()) {
                logger.debug(new StringBuilder(
                        ...
                        .append(mChatId)
                        .append("' as group chat feature has been disabled by the operator.").toString());
            }
            return false;
        }
        ...
        /*
         * TODO : This check will be removed in CR018 implementation as we will
         * be be allowed to add participants even if session is not established
         * right now.
         */
        ...
        if (groupChat == null) {
            if (logger.isActivated()) {
                logger.debug(new StringBuilder(
                        ...
                        .append(mChatId).append("' as the group chat does not exist in DB.")
                        .toString());
            }
            return false;
        }
    ...
    public boolean isAllowedToInviteParticipant(ContactId participant) {
        ...
        /*
         * TODO: Participant's current status has to be considered to decide if
         * he/she can be invited to this group chat.
         */
        return true;
    }

...
public class GeolocSharingImpl extends IGeolocSharing.Stub implements GeolocTransferSessionListener {
    ...
    public Geoloc getGeoloc() {
        ...
        if (session == null) {
            /*
             * TODO : Use persisted storage access GeolocSharing provider implemented in CR025
             */
            return null;
        }
    ...
    private void handleSessionRejected(int reasonCode) {
        ...
        synchronized (lock) {
            ...
            mBroadcaster.broadcastGeolocSharingStateChanged(getRemoteContact(), mSharingId,
                    GeolocSharing.State.REJECTED, reasonCode);
        }
    }
    ...
    public void handleSessionStarted() {
        ...
        synchronized (lock) {
            ...
            mBroadcaster.broadcastGeolocSharingStateChanged(getRemoteContact(), getSharingId(),
                    GeolocSharing.State.STARTED, ReasonCode.UNSPECIFIED);
        }
    }
    ...
    public void handleContentTransfered(GeolocPush geoloc) {
        ...
        synchronized (lock) {
            ...
            if (session instanceof OriginatingGeolocTransferSession) {
                ...
                /*
                 * TODO: Important notice. This will be changed with CR025, as geoloc sharing object
                 * will not be persisted in the message log.
                 */
            } else {
                MessagingLog.getInstance().addIncomingOneToOneChatMessage(geolocMsg);
            }

            mBroadcaster.broadcastGeolocSharingStateChanged(contact, mSharingId,

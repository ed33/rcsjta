...
public abstract class IPCallSession extends ImsServiceSession {
        ...
        if (reInvite.getSdpContent() == null) {
            ...
            switch (requestType) {
                ...
                case (2): { // Case Set On Hold Inactive
                    ...
                    // Notify listeners
                    for (int i = 0; i < getListeners().size(); i++) {
                        ((IPCallStreamingSessionListener) getListeners().get(i)).handleCallHold();
                    }
                }
                    break;
                ...
                case (5): { // Case Set On Resume
                    ...
                    // Notify listeners
                    for (int i = 0; i < getListeners().size(); i++) {
                        ((IPCallStreamingSessionListener) getListeners().get(i)).handleCallResume();
                    }
                }
                    break;
            }
    ...
    public void handleReInviteAck(int code, int requestType) {
        ...
        if ((requestType == IPCallSession.ADD_VIDEO) && (code == 200)) {
            ...
            // Notify listeners
            for (int i = 0; i < getListeners().size(); i++) {
                ((IPCallStreamingSessionListener) getListeners().get(i)).handleAddVideoAccepted();
            }
            ...
            // Notify listeners
            for (int i = 0; i < getListeners().size(); i++) {
                ((IPCallStreamingSessionListener) getListeners().get(i)).handleCallHoldAccepted();
            }
            ...
            // Notify listeners
            for (int i = 0; i < getListeners().size(); i++) {
                ((IPCallStreamingSessionListener) getListeners().get(i)).handleCallResumeAccepted();
            }
            // release hold manager
            holdMgr = null;
        }
            ...
            if (getPlayer() == null) {
                ...
                if (selectedVideoCodec == null) {
                    ...
                    sdp = "v=0" + SipUtils.CRLF + "o=- " + ntpTime + " " + ntpTime + " "
                            ...
                            + SipUtils.CRLF + "t=0 0" + SipUtils.CRLF + audioSdp + "a=sendrcv"
    ...
    protected class RendererEventListener extends IIPCallRendererListener.Stub {
        ...
        public void onRendererError(int error) {
            ...
            for (int i = 0; i < getListeners().size(); i++) {
                ((IPCallStreamingSessionListener) getListeners().get(i))
                        .handleCallError(new IPCallError(IPCallError.RENDERER_FAILED));
            }

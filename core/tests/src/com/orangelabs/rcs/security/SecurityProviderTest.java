...
public class SecurityProviderTest extends AndroidTestCase {
	...
	public void testAddAuthorization() {
		...
		mSecurityInfos.addAuthorization(mAuth1);		
		Integer id = mSecurityInfosTest.getIdForPackageUidAndIari(
		...
		mSecurityInfos.addAuthorization(mAuth1);
		Integer new_id = mSecurityInfosTest.getIdForPackageUidAndIari(
				mContentResolver, UID1, "urn:urn-7:3gpp-application.ims.iari.rcs.mnc099.mcc099.demo1");
		assertEquals(id, new_id);
	...
	public void testRevocations() {
		Map<RevocationData, Integer> revocationDatas = mSecurityInfosTest.getAllRevocations(mContentResolver);				

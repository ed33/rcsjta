...
public class ChatServiceConfigurationTest extends AndroidTestCase {
    ...
    public void testChatServiceConfiguration() {
        ChatServiceConfiguration config = new ChatServiceConfiguration(imAlwaysOn, warnSF,
                chatTimeout, isComposingTimeout, maxGroupChatParticipants,
                minGroupChatParticipants, maxMsgLengthSingleChat, maxMsgLengthGroupChat,
                groupChatSubjectMaxLength, smsFallback, respondToDisplayReports,
                maxGeolocLabelLength, geolocExpireTim);
        Parcel parcel = Parcel.obtain();
        config.writeToParcel(parcel, 0);
        // done writing, now reset parcel for reading
        parcel.setDataPosition(0);
        // finish round trip
        ChatServiceConfiguration createFromParcel = ChatServiceConfiguration.CREATOR
        ...
        assertEquals(createFromParcel.getGroupChatMaxParticipants(),
                config.getGroupChatMaxParticipants());
        assertEquals(createFromParcel.getGroupChatMinParticipants(),
                config.getGroupChatMinParticipants());
        assertEquals(createFromParcel.getGroupChatMessageMaxLength(),
                config.getGroupChatMessageMaxLength());
        assertEquals(createFromParcel.getGroupChatSubjectMaxLength(),
                config.getGroupChatSubjectMaxLength());
        assertEquals(createFromParcel.getOneToOneChatMessageMaxLength(),
                config.getOneToOneChatMessageMaxLength());
        assertEquals(createFromParcel.isSmsFallback(), config.isSmsFallback());
        assertEquals(createFromParcel.isRespondToDisplayReportsEnabled(),
                config.isRespondToDisplayReportsEnabled());
